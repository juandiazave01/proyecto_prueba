...
// ============================================
// NESTED SUB-TABS (generic — works for any .sub-tabs-wrap group)
// ============================================
document.querySelectorAll('.sub-tabs-wrap').forEach(wrap => {
  wrap.querySelectorAll('.sub-tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      wrap.querySelectorAll('.sub-tab-btn').forEach(b => {
        b.classList.toggle('active', b.dataset.subtopic === subtopic);
      });
      wrap.querySelectorAll('.sub-panel').forEach(p => {
        p.classList.toggle('active', p.dataset.subtopic === subtopic);
      });
    });
  });
});
...
// ============================================
// ARIA DEMO — auto-playing simulated chat (illustrative, fictional data)
// ============================================
const ariaDemoChat = document.getElementById('aria-demo-chat');

if (ariaDemoChat) {
  const ariaScenarios = [
    [
      { who: 'user', text: "What's the tag for Dryer Zone 3 temperature?" },
      { who: 'aria', text: "Zone 3 RTD reads on DB120.DBW30, Cabinet 7080 (Dryer & Calender). PID setpoint is DB120.DBD40 if you need that too." }
    ],
    [
      { who: 'user', text: "Packaging line stopped, no fault code on the HMI." },
      { who: 'aria', text: "Let's check the basics: 1) E-stop chain clear? 2) Air pressure OK on the packaging skid? 3) DB560.DBX0.0 shows a latched interlock the HMI isn't surfacing. Want the wiring diagram for that circuit?" }
    ],
    [
      { who: 'user', text: "Show me the safety relay for the card guard." },
      { who: 'aria', text: "PSR relay, Cabinet 30, channel 2 — documented in FOR_CARD_Safety_Rev3. Category 3 stop, dual-channel monitored. Want the reset sequence?" }
    ],
    [
      { who: 'user', text: "Log this to shift notes: replaced photo eye on Buff 2." },
      { who: 'aria', text: "Logged — 14:32, Buff 2 photo eye replacement. This is the third one this month. Want me to flag it for the reliability review?" }
    ]
  ];
  ...
  async function typeMessage(who, fullText) {
    const msg = document.createElement('div');
    msg.className = 'aria-msg ' + (who === 'user' ? 'aria-msg-user' : 'aria-msg-aria');
    const label = document.createElement('span');
    label.className = 'aria-msg-label';
    label.textContent = who === 'user' ? 'Technician' : 'ARIA';
    const body = document.createElement('span');
    const cursor = document.createElement('span');
    cursor.className = 'aria-typing-cursor';

    msg.appendChild(label);
    msg.appendChild(body);
    msg.appendChild(cursor);
    ariaDemoChat.appendChild(msg);
    ariaDemoChat.scrollTop = ariaDemoChat.scrollHeight;

    const speed = who === 'user' ? 22 : 14;
    for (let i = 0; i < fullText.length; i++) {
      if (cancelled) return;
      body.textContent += fullText.charAt(i);
      ariaDemoChat.scrollTop = ariaDemoChat.scrollHeight;
      await sleep(speed);
    }
    cursor.remove();
  ...
  async function playAriaDemo() {
    if (isPlaying) return;
    isPlaying = true;
    while (!cancelled) {
      const scenario = ariaScenarios[scenarioIndex % ariaScenarios.length];
      ariaDemoChat.innerHTML = '';

      for (const line of scenario) {
        await sleep(line.who === 'user' ? 400 : 500);
        await typeMessage(line.who, line.text);
      }
  ...
  // Only run the animation while the demo is actually visible on screen
  const ariaObserver = new IntersectionObserver((entries) => {
  ...
  ariaObserver.observe(ariaDemoChat);
}
